...
async function previousCompletedForContract(contractId: number | null | undefined, beforeNumber: number) {
    if (!contractId) {
        return null;
    }
    const rows = await db
...
export async function upsertCustomer(input: z.infer<typeof customerSchema>): Promise<ActionResult<{customerId: number}>> {
    ...
    const salespersonId = parsed.data.salespersonId ?? (session.user.role === "salesperson" ? session.user.id : null);
...
export async function createContract(input: {
    ...
    const session = await requireActionRole(["admin", "salesperson"]);
    const [existing] = await db.select().from(contract).where(eq(contract.customerId, input.customerId)).limit(1);
    if (existing?.locked) {
    ...
    const [created] = await db
        ...
        .values({
            ...
            salespersonId: input.salespersonId ?? (session.user.role === "salesperson" ? session.user.id : null),
            serviceType: input.serviceType,
            contractValue: input.contractValue,
            paymentStatus: PAYMENT_STATUSES.includes(input.paymentStatus as (typeof PAYMENT_STATUSES)[number])
                ? input.paymentStatus
                : "Pending",
            paymentFrequency: PAYMENT_FREQUENCIES.includes(input.paymentFrequency as (typeof PAYMENT_FREQUENCIES)[number])
...
export async function scheduleService(input: {
    ...
    let contractRow = input.contractId
        ...
        : (await db.select().from(contract).where(eq(contract.customerId, input.customerId)).limit(1))[0];
...
export async function createComplaint(input: {
    ...
    const [serviceRow] = await db.select().from(service).where(eq(service.id, input.serviceId)).limit(1);
    if (!serviceRow) {
        return fail("Service not found");
    }
    ...
    const [created] = await db
        ...
        .values({
            ...
            customerId: input.customerId,
            complaintType: COMPLAINT_TYPES.includes(input.complaintType as (typeof COMPLAINT_TYPES)[number])
                ? input.complaintType
                : "Service quality",
            priority: COMPLAINT_PRIORITIES.includes(input.priority as (typeof COMPLAINT_PRIORITIES)[number])
                ? input.priority
                : "Normal",
            status: "Unscheduled",
...
export async function assignRedo(input: {
    ...
    await requireActionRole(["admin"]);
    const [row] = await db.select().from(complaint).where(eq(complaint.id, input.complaintId)).limit(1);
    if (!row) {
...
export async function verifyService(serviceId: number, requestRework?: boolean): Promise<ActionResult> {
    await requireActionRole(["admin", "salesperson"]);
    await db
        .update(service)
