...
function permissionIdsForRole(role: string) {
    if (role === "admin") {
        return ["dashboard", "users", "customers", "services", "reports", "settings", "complaints", "notifications", "contracts", "billing"];
    }
    if (role === "salesperson") {
...
export async function createStaffUser(input: z.infer<typeof createUserSchema>): Promise<ActionResult<{id: string}>> {
    ...
    await db.insert(userPermission).values(
        permissionIdsForRole(role).map((permissionId) => ({userId: id, permissionId})),
    );
...
export async function updateStaffUser(input: z.infer<typeof updateUserSchema>): Promise<ActionResult> {
    await requireActionRole(["admin"]);
...
export async function updateStaffPermissions(input: {
    ...
    if (input.permissions.length > 0) {
        await db.insert(userPermission).values(input.permissions.map((permissionId) => ({userId: input.id, permissionId})));
    }
    revalidatePath("/admin/user-management");
    return okEmpty();
}
