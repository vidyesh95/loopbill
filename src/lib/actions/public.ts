...
export async function submitPublicComplaint(input: z.infer<typeof phoneSchema>) {
    ...
    if (!isComplaintSameMonth(serviceDate, raisedAt)) {
        return {ok: false as const, error: "Complaints must be raised in the same calendar month as the service."};
    }
...
export async function submitPublicReschedule(input: {phone: string; requestedDate?: string; reason: string}) {
    ...
    const [recent] = owner
        ? await db.select().from(service).where(eq(service.customerId, owner.id)).orderBy(desc(service.scheduledAt)).limit(1)
        : [];
    ...
    if (recent) {
        await db.update(service).set({status: "Reschedule required"}).where(eq(service.id, recent.id));
    }
