...
export async function completeAssignedService(input: {
  ...
  const previous = row.contractId
    ? (await db.select().from(service).where(eq(service.contractId, row.contractId))).find(
        (item) => item.status === "Completed" && item.serviceNumber < row.serviceNumber,
      )
    : undefined;
...
export async function reportCustomerAbsence(
  serviceId: number,
  reason?: string,
): Promise<ActionResult> {
  const session = await requireActionRole(["agent", "admin"]);
