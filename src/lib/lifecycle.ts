...
export function assertServiceDate(input: {
  ...
  if (!input.previousCompletedAt) {
    return {
      ok: false as const,
      error: "Previous service must be completed before scheduling the next visit.",
    };
  }
...
export function complaintVisibleToAdmin(
  visibleToAdminAt: Date | null | undefined,
  now = new Date(),
) {
  if (!visibleToAdminAt) {
    return true;
...
export function redoEligible(input: {
  ...
  if (input.existingRedoId) {
    return {
      ok: false as const,
      error: "Only one redo is allowed per service unless an administrator overrides.",
    };
  }
  if (!input.attendedAt) {
    return {
      ok: false as const,
      error: "Complaint must be attended before a redo can be scheduled.",
    };
  }
