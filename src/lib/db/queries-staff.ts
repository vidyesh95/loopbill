...
export async function getJobs(filter?: {agentId?: string; salespersonId?: string; statuses?: string[]}): Promise<JobRecord[]> {
    const rows = await db
        .select({
            id: service.id,
...
export async function getContractsAtRisk() {
    const rows = await db
        ...
        .where(or(eq(contract.status, "Expiring Soon"), eq(contract.locked, true), eq(contract.status, "Active")));
...
export async function getLookups() {
    const [customers, agents, packages] = await Promise.all([
        db.select({id: customer.id, name: customer.name, phone: customer.phone}).from(customer).orderBy(customer.name),
        getAgents(),
        getPackages(),
    ]);
