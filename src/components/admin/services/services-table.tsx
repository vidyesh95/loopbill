      ...
      <TableBody>
        {services.map((service) => (
          <TableRow key={service.id}>
            ...
            <TableCell>
              <span
                className={`rounded-full border px-2 py-0.5 font-semibold ${getStatusColor(service.status)}`}
              >
                {service.status}
              </span>
            </TableCell>
...
export default function ServicesTable({ services = allServices }: { services?: ServiceRow[] }) {
  ...
  const [allServicesSelectedServiceType, setAllServicesSelectedServiceType] =
    useState<string>("all");
  ...
  const allServicesFilteredByType =
    allServicesSelectedServiceType === "all"
      ? allServicesByStatus
      : allServicesByStatus.filter(
          (service) => service.serviceType === allServicesSelectedServiceType,
        );

  // Logic for 'upcoming-services' tab
  const upcomingServicesByStatus = filterServicesStatus(services, "upcoming-services");
  const upcomingServicesFilteredByType =
    upcomingSelectedServiceType === "all"
      ? upcomingServicesByStatus
      : upcomingServicesByStatus.filter(
          (service) => service.serviceType === upcomingSelectedServiceType,
        );

  // Logic for 'completed-services' tab
  const completedServicesByStatus = filterServicesStatus(services, "completed-services");
  const completedServicesFilteredByType =
    completedSelectedServiceType === "all"
      ? completedServicesByStatus
      : completedServicesByStatus.filter(
          (service) => service.serviceType === completedSelectedServiceType,
        );

  // Logic for 'redo-services' tab
  const redoServicesByStatus = filterServicesStatus(services, "redo-services");
  const redoServicesFilteredByType =
    redoSelectedServiceType === "all"
  ...
  const expiredServicesFilteredByType =
    expiredSelectedServiceType === "all"
      ? expiredServicesByStatus
      : expiredServicesByStatus.filter(
          (service) => service.serviceType === expiredSelectedServiceType,
        );

  return (
    <Tabs defaultValue="all-services" className="w-full">
      <TabsList className="grid w-full grid-cols-5">
        <TabsTrigger value="all-services" className="cursor-pointer">
          All services
        </TabsTrigger>
        <TabsTrigger value="upcoming-services" className="cursor-pointer">
          Upcoming services
        </TabsTrigger>
        <TabsTrigger value="completed-services" className="cursor-pointer">
          Completed services
        </TabsTrigger>
        <TabsTrigger value="redo-services" className="cursor-pointer">
          Redo services
        </TabsTrigger>
        <TabsTrigger value="expired-services" className="cursor-pointer">
          Expired services
        </TabsTrigger>
      </TabsList>
      <TabsContent value="all-services">
        <Card>
          <CardHeader>
            ...
            <CardDescription>View and manage all pest control services</CardDescription>
            <FilterByServiceType
              value={allServicesSelectedServiceType}
              onValueChange={setAllServicesSelectedServiceType}
            />
          </CardHeader>
          <CardContent>{renderServices(allServicesFilteredByType)}</CardContent>
          <CardFooter>
      ...
      <TabsContent value="upcoming-services">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Services scheduled or in progress</CardDescription>
            <FilterByServiceType
              value={upcomingSelectedServiceType}
              onValueChange={setUpcomingSelectedServiceType}
            />
          </CardHeader>
          <CardContent>{renderServices(upcomingServicesFilteredByType)}</CardContent>
          <CardFooter>
            <ServicesTablePagination />
          </CardFooter>
        </Card>
      ...
      <TabsContent value={"completed-services"}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>Services that have been completed successfully</CardDescription>
            <FilterByServiceType
              value={completedSelectedServiceType}
              onValueChange={setCompletedSelectedServiceType}
            />
          </CardHeader>
          <CardContent>{renderServices(completedServicesFilteredByType)}</CardContent>
          <CardFooter>
            <ServicesTablePagination />
          </CardFooter>
        </Card>
      ...
      <TabsContent value={"redo-services"}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>Services that need to be redone due to complaints</CardDescription>
            <FilterByServiceType
              value={redoSelectedServiceType}
              onValueChange={setRedoSelectedServiceType}
            />
          </CardHeader>
          <CardContent>{renderServices(redoServicesFilteredByType)}</CardContent>
          <CardFooter>
            <ServicesTablePagination />
          </CardFooter>
        </Card>
      ...
      <TabsContent value={"expired-services"}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>Services that have expired without completion</CardDescription>
            <FilterByServiceType
              value={expiredSelectedServiceType}
              onValueChange={setExpiredSelectedServiceType}
            />
          </CardHeader>
          <CardContent>{renderServices(expiredServicesFilteredByType)}</CardContent>
          <CardFooter>
            <ServicesTablePagination />
          </CardFooter>
        </Card>
